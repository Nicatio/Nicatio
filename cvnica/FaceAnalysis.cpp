...
void FaceAnalysis::grayminmaxfloat(
		InputArray 						_src,
		OutputArray						_dst,
		const double					min,
		const double					max)
{
	...
	minMaxIdx(srcfloat,&Tmin,&Tmax);

	cout<<"d"<<Tmin<<", "<<Tmax<<", "<<sum(srcfloat)[0]<<endl;
	...
	for (int i=0;i<nImages;i++) {
		...
		for (int j=0;j<nSubject;j++) {
			if (!BadImage.at<char>(i)) {
				...
				if(i/nPicsPerSubject == j) {analysisImageIntra[j] += multResult;counter++;}//cout<<"a-"<<sum(multResult)[0]<<"\t";}
									  else {analysisImageInter[j] += multResult;counter2++;}//cout<<"b-"<<sum(multResult)[0]<<"\t";}
		...
		const int						bin)
{
	...
	of.open("intra.txt",ios::binary);
	of2.open("inter.txt",ios::binary);

	for (int i=0;i<nSubject;i++)
		ambienceImage[i] = imread( DirectoryLocation+"/"+files[i*nPicsPerSubject],CV_8UC1);
	...
	int counter2=0;
	for (int i=0;i<nImages;i++) {
		Mat testImage = imread( DirectoryLocation+"/"+files[i],CV_8UC1);
		for (int j=0;j<nSubject;j++) {
			if (!BadImage.at<char>(i)) {
				if(i/nPicsPerSubject == j && i%nPicsPerSubject==0) continue;
				Mat diffResult, sq;
				absdiff(testImage,ambienceImage[j],diffResult);
				sq = diffResult.mul(diffResult);
				if(i/nPicsPerSubject == j) {
					of<<sqrt((double)sum(sq)[0])<<endl;
					counter++;
				} else {
					of2<<sqrt((double)sum(sq)[0])<<endl;
					counter2++;
				}
			}
		}
	}
	cout<<"intra: "<<counter<<endl;
	cout<<"inter: "<<counter2<<endl;
	of.close();
	of2.close();
}
